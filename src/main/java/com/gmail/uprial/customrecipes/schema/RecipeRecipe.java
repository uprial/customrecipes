...
public final class RecipeRecipe {
    private final String[] recipe;

    private RecipeRecipe() {
        recipe = new String[9];
        for(int i = 0; i < 9; i++) {
    ...
    public ShapedRecipe getShapedRecipe(CustomRecipes plugin, String keyLC, ItemStack result) {
    ...
    public static RecipeRecipe getFromConfig(FileConfiguration config, String key, String title) throws InvalidConfigException {
        RecipeRecipe recipe = new RecipeRecipe();

        List<?> rows = config.getList(key);
        if(rows == null) {
